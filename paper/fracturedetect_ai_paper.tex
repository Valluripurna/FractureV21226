...
\newcommand{\napkinimage}[2][]{\includegraphics[clip,trim=0 14 14 0,#1]{#2}}
...
\begin{figure}[htbp]
\centerline{\napkinimage[width=0.5\textwidth]{system_architecture.png}}
\caption{Overall System Architecture. The user-facing React frontend communicates with Flask-based backend services, which orchestrate preprocessing, model inference, and result aggregation. A MongoDB layer persists users, studies, and reports.}
...
\centerline{\napkinimage[width=0.45\textwidth]{frontend_interface.png}}
\caption{Frontend Interface Mockup. The UI supports authentication, image upload, progress feedback, AI results with confidence, report downloads, chat assistance, and history review.}
...
\centerline{\napkinimage[width=0.45\textwidth]{model_performance.png}}
\caption{Model Performance Comparison. Bar chart of per-model accuracy on the evaluation set; EfficientNet-B0 achieves the highest accuracy among the ensemble members.}
...
\centerline{\napkinimage[width=0.45\textwidth]{processing_pipeline.png}}
\caption{Detailed System Architecture and Processing Flow. The pipeline covers ingestion, preprocessing, standardized transforms, batched inference, post-processing, interpretation, and reporting.}
...
\centerline{\napkinimage[width=0.45\textwidth]{xray_samples.png}}
\caption{Sample X-ray Images from Dataset. Representative views illustrate variability in anatomy, projection, and image quality that the models must handle.}
...
\begin{align}
\Large \mathrm{Accuracy} &= \frac{TP + TN}{TP + TN + FP + FN} \\
\Large \mathrm{Precision} &= \frac{TP}{TP + FP} \\
\Large \mathrm{Recall} &= \frac{TP}{TP + FN} \\
\Large \mathrm{F1\text{-}Score} &= 2\,\times\,\frac{\mathrm{Precision}\,\times\,\mathrm{Recall}}{\mathrm{Precision}+\mathrm{Recall}}
\end{align}
...
\centerline{\napkinimage[width=0.4\textwidth]{confusion_matrix.png}}
\caption{Confusion Matrix for Fracture Detection. Diagonal cells are correct predictions; off-diagonals indicate errors. The distribution reflects the precision–recall balance observed.}
...
\begin{table}[htbp]
\caption{AUC Values for Different Models}
\begin{center}
\begin{tabular}{|c|c|}
\hline
...
\centerline{\napkinimage[width=0.45\textwidth]{roc_curves.png}}
\caption{ROC Curves for All Models. Curves illustrate sensitivity–specificity tradeoffs across thresholds; EfficientNet-B0 attains the highest AUC.}
